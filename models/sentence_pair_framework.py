...
class sentence_pair_framework(nn.Module):
    def __init__(self, data, config):
        ...
        self.encoder = encoder_fn(config) #enter
    ...
    def forward(self, batch):
        ...
        if "batch_pair" in self.config and self.config["batch_pair"]:
            ...
            sequence_detach = sequence.detach()
            sequence_dict = self.encoder(sequence_detach, input_mask) #enter
            #sequence_dict = self.encoder(sequence, input_mask)  # enter
            #print(f"sequence = {sequence_dict['sequence'].shape}, global_state = {sequence_dict['global_state'].shape}, input_mask = {sequence_dict['input_mask'].shape}")
            sequence1_dict = {}
            sequence2_dict = {}
